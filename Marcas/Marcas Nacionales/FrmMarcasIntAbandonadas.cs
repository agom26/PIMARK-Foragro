...
{
    public partial class FrmMarcasIntAbandonadas : Form, IAsyncLoadable
    {
        ...
        public async Task filtrar()
        {
            ...
            {
                totalRows = await marcaModel.GetFilteredMarcasEnAbandonoCount(buscar);
                totalPages = (int)Math.Ceiling((double)totalRows / pageSize);
                lblTotalPages.Text = totalPages.ToString();
                lblTotalRows.Text = totalRows.ToString();

                DataTable dt = await marcaModel.FiltrarMarcasEnAbandono(buscar, currentPageIndex, pageSize);
                if (dt.Rows.Count > 0)
                {
