...
{
    public partial class FrmRenovacionesInt : Form, IAsyncLoadable
    {
        ...
        public async Task filtrar()
        {
            ...
            {
                totalRows = await marcaModel.GetFilteredMarcasEnTramiteDeRenovacionCount(txtBuscar.Text); totalPages = (int)Math.Ceiling((double)totalRows / pageSize);
                lblTotalPages.Text = totalPages.ToString();
                lblTotalRows.Text = totalRows.ToString();

                DataTable dt = await marcaModel.FiltrarMarcasNacionalesEnTramiteDeRenovacion(buscar, currentPageIndex, pageSize);
   ...
   ref byte[] logo, bool registroChek, string registro, string folio, string libro)
        ...
        public async void filtrar()
        {
            ...
            if (!string.IsNullOrEmpty(buscar))
            {
                try
                {
                    totalRows = await marcaModel.GetFilteredMarcasEnTramiteDeRenovacionCount(buscar);
                    totalPages = (int)Math.Ceiling((double)totalRows / pageSize);
                    lblTotalPages.Text = totalPages.ToString();
                    lblTotalRows.Text = totalRows.ToString();
        ...
        private async void roundedButton4_Click_1(object sender, EventArgs e)
        {
            await ListarArchivosEnGeneral();
        }
