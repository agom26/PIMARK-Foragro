...
namespace Presentacion.Marcas_Nacionales
{
    ...
    public partial class FrmMostrarTodas : Form, IAsyncLoadable
    {
        ...
        public FrmMostrarTodas()
        {
            ...
            if (UsuarioActivo.soloLectura)
            {
                ...
                btnEnviarAOposicion.Visible = false;
            ...
            else if (!UsuarioActivo.soloLectura)
            {
                ...
                btnEnviarAOposicion.Visible = true;
                btnCancelarM.Visible = true;
        ...
        private async Task LoadMarcas()
        {
            ...
            var marcasN = await marcaModel.GetAllMarcasInternacionalesIngresadas(currentPageIndex, pageSize);
        ...
        public async Task filtrar()
        {
            ...
            {
                totalRows = await marcaModel.GetFilteredMarcasInternacionalesSinRegistroCount(buscar);
                totalPages = (int)Math.Ceiling((double)totalRows / pageSize);
                lblTotalPages.Text = totalPages.ToString();
                lblTotalRows.Text = totalRows.ToString();

                DataTable dt = await marcaModel.FiltrarMarcasInternacionalesEnTramite(buscar, currentPageIndex, pageSize);
                if (dt.Rows.Count > 0)
                {
                    dtgMarcasN.DataSource = dt;
    ...
    ref byte[] logo, bool registroChek, string registro, string folio, string libro)
        ...
        public async Task ActualizarMarcaInternacional()
        {
            ...
            try
            {
                ...
                bool esActualizado = registroChek ?
                    await marcaModel.EditMarcaInternacionalRegistradaNuevo(
                        SeleccionarMarca.idInt, expediente, nombre, signoDistintivo, tipoSigno, clase, multiclase, logo, idTitular, idAgente, solicitud, paisRegistro, tiene_poder, idCliente, registro, folio, libro, fecha_registro, indefinida, fecha_vencimiento, "", "", ubicacionF) :
                    await marcaModel.EditMarcaInternacionalNuevo(SeleccionarMarca.idInt, expediente, nombre, signoDistintivo, tipoSigno, clase, multiclase, logo, idTitular, idAgente, solicitud, paisRegistro, tiene_poder, idCliente, ubicacionF);
                ...
                if (esActualizado)
                {
                    ...
                    {
                        await historialModel.GuardarEtapa(SeleccionarMarca.idInt, Convert.ToDateTime(AgregarEtapa.fecha), AgregarEtapa.etapa, AgregarEtapa.anotaciones, UsuarioActivo.usuario, "TRÁMITE", AgregarEtapa.fechaVencimiento);
                        agregoEstado = false;
        ...
        private async Task SubirArchivoRegistro(string idMarca)
        {
        ...
        private async void dtgMarcasN_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            await EditarVerHistorial();
        }
        ...
        private async void btnAdjuntarT_Click(object sender, EventArgs e)
        {
            if (!UsuarioActivo.soloLectura)
            {
                await SubirArchivoRegistro("" + SeleccionarMarca.idInt);
                if (!archivoSubido)
                {
