...
{
    public partial class FrmMostrarDesistidas: Form, IAsyncLoadable
    {
        ...
        {
            totalRows = await marcaModel.GetTotalMarcasInternacionalesEnDesistimiento();
            totalPages = (int)Math.Ceiling((double)totalRows / pageSize);

            // Obtiene las marcas
            var marcasN = await marcaModel.GetAllMarcasInternacionalesEnDesistimiento(currentPageIndex, pageSize);
        ...
        public async Task filtrar()
        {
            ...
            {
                totalRows = await marcaModel.GetFilteredMarcasInternacionalesEnDesistimientoCount(buscar);
                totalPages = (int)Math.Ceiling((double)totalRows / pageSize);
                lblTotalPages.Text = totalPages.ToString();
                lblTotalRows.Text = totalRows.ToString();

                DataTable dt = await marcaModel.FiltrarMarcasInternacionalesEnDesistimiento(buscar, currentPageIndex, pageSize);
                if (dt.Rows.Count > 0)
                {
        ...
        public async void filtrar()
        {
            ...
            if (!string.IsNullOrWhiteSpace(buscar))
            {
                try
                {
                    ...
                    DataTable titulares = await marcaModel.FiltrarMarcasInternacionalesEnDesistimiento(buscar, currentPageIndex, pageSize);
                    ...
                    if (this.IsHandleCreated && !this.IsDisposed)
                    {
                        this.Invoke(() =>
                        {
                            ...
                            else
                            {
                                ...
                                _ = LoadMarcas(); // sin await porque ya estás en void
                            }
                        });
                    }
                }
        ...
        private async void FrmMostrarDesistidas_Load(object sender, EventArgs e)
        {
   ...
   ref byte[] logo, bool registroChek, string registro, string folio, string libro)
        ...
        private void FrmMostrarDesistidas_Resize(object sender, EventArgs e)
        {
            CentrarPanel();
        }
    }
